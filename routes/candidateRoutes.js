...
candidateRouter.get('/id=:id', authorizedOrdinaryUser, async (req, res) => {
    ...
    if (candidateId === undefined || candidateId === null || candidateId < 0 || isNaN(candidateId) || Number.isNaN(candidateId)) {
...
candidateRouter.put('/update/id=:id', authorizedAdmin, upload.single('file'), async (req, res) => {
    ...
    if(!req.params.id || !req.body.name || !req.body.message || isNaN(req.params.id) || Number.isNaN(req.params.id)) {
...
candidateRouter.delete('/delete/id=:id', authorizedAdmin, async (req, res) => {
    ...
    if (!candidateId || candidateId < 0 || isNaN(candidateId) || Number.isNaN(candidateId)) {
        logger.error(`Candidate id ${candidateId} is invalid!`);
